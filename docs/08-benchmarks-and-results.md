...
| Watch (3c) | pure `evaluate()` — warn/critical below the free-space threshold (bytes or %), growth alert past a delta, "here's how much you'd get back" suggestion · `Monitor.check()` measures growth against recorded history · `[watch]` config fail-safe · `reclaim watch --once` healthy vs alerting; notifications never fire in tests |
| Dashboard (3d) | pure `build_dashboard()` renders every section (summary/disk/top-units/projects/trend) · disk bar flips below vs above the threshold · empty scan and missing-history both degrade gracefully · `reclaim dashboard` one-shot exits clean |
...
<!-- Phase 3 complete. Future work (e.g. the Rust/PyO3 hot path) will be appended as it lands. -->
